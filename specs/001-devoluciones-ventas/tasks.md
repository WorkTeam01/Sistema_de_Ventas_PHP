...
- [x] T1. Añadir `tb_devoluciones` y `tb_devolucion_items` a
...
- [x] T2. Añadir las mismas dos tablas a `tests/fixtures/schema.sqlite.sql`
...
- [x] T3. Crear `database/migrations/007_devoluciones.sql` con:
...
- [x] T4. Actualizar `database/seeder.sql`: permiso `manage_returns` en la
...
- [x] T5. Modelo `App\Models\SaleReturn`: `pendingByVenta()`, `findWithDetails()`,
...
- [x] T6. `Sale::isReferenced(int|string $id): bool` (consulta a
...
- [x] T7. Agregaciones netas del Dashboard (FR-21): `Sale::totalCurrentMonth`,
...
- [x] T8. Agregaciones netas de Report (FR-21): `salesByPeriod`, `salesTotals`,
...
- [x] T9. Indicador de devoluciones (FR-22): `Sale::allWithDetails()` expone
...
- [x] T10. `views/sales/show.php`: sección "Devoluciones" con `byVenta()` y
...
- [x] T11. `SaleReturnController` (index/create/store/show) + scoping FR-15
...
- [x] T12. Vista `views/returns/create.php`: info de la venta (nro, cliente,
...
- [x] T13. Vista `views/returns/index.php` (DataTables + export
...
- [x] T14. `ActivityLogRenderer`: labels `motivo`, `monto_devuelto`,
...
- [x] T15. Tests de integración de devoluciones (`tests/Integration/Models/
...
- [x] T16. Tests de agregaciones netas (`SaleReturnNetTest` o ampliar los
      tests existentes de `Sale`/`Product`/`Report`) con V6..V8 del plan.
      (FR-21, FR-22)
      Done when: V6..V8 pasan (dashboard, reportes, indicador).

- [x] T17. `composer test` completo verde (Unit + Integration; las variantes
...
- [x] Verificar todos los FR (`/sdd:validate` → `specs/001-devoluciones-ventas/validation.md`).
- [x] Mover la feature a `Hecho ✅` en `docs/roadmap.md` enlazando esta carpeta.
- [x] `CHANGELOG.md`: entrada en `[Unreleased]` — tablas `tb_devoluciones` +
      `tb_devolucion_items`, slug `manage_returns`, venta neta en dashboard y
      reportes, y el paso de migración `007_*.sql`.
- [x] Guía de actualización: documentar que las BD existentes deben correr
      `database/migrations/007_devoluciones.sql` (mismo flujo que `001..006`).
- [x] `AGENTS.md`: actualizar la firma de BD (`+ tb_devoluciones`,
