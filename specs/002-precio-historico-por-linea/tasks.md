...
- [x] T1. Añadir `precio_unitario DECIMAL(10,2) NULL` a `tb_carrito` en
...
- [x] T2. Añadir `precio_unitario NUMERIC` a `tb_carrito` en
...
- [x] T3. Crear `database/migrations/006_carrito_precio_unitario.sql` con el
...
- [x] T4. En `Sale::storeWithStock`, antes del INSERT de cabecera: `UPDATE
...
- [x] T5. En `Sale::storeWithStock`, reemplazar el cálculo de `$totalReal` por
...
- [x] T6. En `Sale::findWithDetails`, cambiar `al.precio_venta` por
      `COALESCE(car.precio_unitario, al.precio_venta) AS precio_venta` en el SELECT de ítems. (FR-3)
...
- [x] T7. En `CartItem::getByNroVenta`, cambiar `al.precio_venta` por
...
- [x] T8. Test de regresión de la factura/subtotales (FR-3): venta de 2 ítems →
...
- [x] T9. Test del backfill (FR-5/6/8): insertar filas de `tb_carrito` con
...
- [x] T10. `composer test` completo verde (Unit + Integration). (FR-7 y regresión
      global)
      Done when: `composer test` termina sin fallos ni errores.

- [x] T11. En `Product::getTopSelling` y `Report::topProducts`, reemplazar
...
- [x] Verificar todos los FR (`/sdd:validate` → `specs/002-precio-historico-por-linea/validation.md`).
- [x] Mover la feature a `Hecho ✅` en `docs/roadmap.md` enlazando esta carpeta.
- [x] `CHANGELOG.md`: entrada en `[Unreleased]` — columna `tb_carrito.precio_unitario`,
...
- [x] Guía de actualización: documentar que las BD existentes deben correr
      ...
      `001..005`).
- [x] `AGENTS.md` §Base de Datos: actualizar la firma de `tb_carrito`
